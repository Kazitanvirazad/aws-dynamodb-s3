...
public class CopyS3ObjectToDynamoDBHandler implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
    ...
    public APIGatewayProxyResponseEvent handleRequest(APIGatewayProxyRequestEvent input, Context context) {
        ...
        if (!StringUtils.isNotBlank(productId)) {
            ...
            if (ProductService.copyProductFromS3ToDynamoDB(productId, profileCredential, bucketName)) {
                responseObject = ResponseObject.builder()
                        .setMessage("Product copied successfully from S3 bucket to DynamoDB!")
                        .build();
